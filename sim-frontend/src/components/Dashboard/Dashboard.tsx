...
const Dashboard = ({ project, actionsTracked, messageActivity, messageDistribution, tokenBalances, uniqueUsersData, userMetrics }: any) => {
  ...
  const processedTokenBalances = useMemo(() => {
    ...
    const others = sortedTokenBalances.slice(5).reduce(
      //@ts-ignore
      (acc, curr) => ({
        address: "Others",
        ...
        <CardContent>
          <div className="space-y-2">
            ...
            <p>
              ...
              {Math.round(
                //@ts-ignore
                messageDistribution.reduce((acc, curr) => acc + curr.count, 0) / messageActivity.length
              )}
            </p>
          </div>
        </CardContent>
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={300}>
            <AreaChart data={messageActivity}>
              ...
              <Tooltip content={<CustomTooltip />} />
              <Legend />
              {actionsTracked.map((action: string, index: number) => (
                <Area type="monotone" dataKey={action} stackId="1" stroke={COLORS[index % COLORS.length]} fill={COLORS[index % COLORS.length]} />
              ))}
              {/* <Area
                  type="monotone"
